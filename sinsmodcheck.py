                    ...
                    if (
                        ...
                        if osname == "posix":
                            subprocess.call(
                                [
                                    "wine",
                                    os.path.join(basegame, "ConvertData_Rebellion.exe"),
                                    datatype,
                                    filename,
                                    filename + ".tmp",
                                    "bin",
...
if os.path.isdir(
    ...
    os.path.expanduser(
        "~/.wine/drive_c/Program Files (x86)/Steam/steamapps/common/Sins of a Solar Empire Rebellion"
    )
):
    basegame = os.path.expanduser(
        "~/.wine/drive_c/Program Files (x86)/Steam/steamapps/common/Sins of a Solar Empire Rebellion"
    )
...
stockpath = os.path.join(basegameplaintext, "GameInfo")
...
def readFile(filename):
    ...
    with open(filename, "r+") as f:
        for line in f:
            if linecount == 0 and line.startswith("BIN"):
                ...
                break
            linecount += 1
            if entityfilename.startswith("Player") and line.strip().startswith(
                "count "
            ):
                if (
                    playercountentity > 0
                    and playercount > 0
                    and playercountentity != playercount
                ):
                    print("\t Found a count of " + str(
                        playercount
                    ) + " but the entityDefName below it has a count of " + str(
                        playercountentity
                    ) + " in " + entityfilename)
            ...
            if 'entityType "ResearchSubject"' in line and "ARTIFACT" not in filename:
                ...
                brushfilename = (
                    line.replace("environmentMapName", "").replace('"', "").strip()
                )
                ...
                brushfilename = (
                    line.replace("environmentIlluminationMapName", "")
                    .replace('"', "")
                    .strip()
                )
                if brushfilename != "" and not [brushfilename, filename] in texturelist:
                ...
                brushfilename = (
                    line.replace("beamGlowTextureName", "").replace('"', "").strip()
                )
                if brushfilename != "" and not brushfilename in texturelist:
                    texturelist.append([brushfilename, filename])
            elif "beamCoreTextureName " in line:
                brushfilename = (
                    line.replace("beamCoreTextureName", "").replace('"', "").strip()
                )
                if brushfilename != "" and not [brushfilename, filename] in texturelist:
                    texturelist.append([brushfilename, filename])
                ...
                stringname = (
                    line.replace("toggleStateOnNameStringID", "")
                    .replace('"', "")
                    .strip()
                )
                if stringname != "" and not [stringname, filename] in stringlist:
                ...
                stringname = (
                    line.replace("toggleStateOnDescStringID", "")
                    .replace('"', "")
                    .strip()
                )
                if stringname != "" and not [stringname, filename] in stringlist:
                ...
                stringname = (
                    line.replace("counterDescriptionStringID", "")
                    .replace('"', "")
                    .strip()
                )
                if stringname != "" and not [stringname, filename] in stringlist:
                ...
                "meshNameIncreasedEffectName" in line
                or "meshNameDecreasedEffectName" in line
            ):
                particlename = line.strip().split()[1].replace('"', "").strip()
                if particlename != "" and not [particlename, filename] in particlelist:
                    particlelist.append([particlename, filename])
                ...
                entityname = (
                    line.replace("cannonShellType", "").replace('"', "").strip()
                )
                if entityname != "" and not [entityname, filename] in entitylinked:
                    entitylinked.append([entityname, filename])
                    if not isModEntity(entityname + ".entity"):
                ...
                if (
                    entityname != ""
                    and entityname != "BuffWormHoleTeleport"
                    and not [entityname, filename] in entitylinked
                ):
                    entitylinked.append([entityname, filename])
                    if not isModEntity(entityname + ".entity"):
                        notInMod(entityname, filename)
                ...
                entityname = (
                    line.replace("buffToApplyOnImpact", "").replace('"', "").strip()
                )
                if entityname != "" and not [entityname, filename] in entitylinked:
                    entitylinked.append([entityname, filename])
                    if not isModEntity(entityname + ".entity"):
                ...
                entityname = (
                    line.replace("fighterEntityDef", "").replace('"', "").strip()
                )
                if entityname != "" and not [entityname, filename] in entitylinked:
                    entitylinked.append([entityname, filename])
                    if not isModEntity(entityname + ".entity"):
                ...
                if (
                    "baseCommandPoints" not in line
                    and "maxNumCommandPoints" not in line
                ):
                    line = next(f)
                value = line.strip().split()[1].replace('"', "")
                if (
                    float(value) > 0
                    and not filename in squadentities
                ...
                entityname = (
                    line.replace("afterColonizeBuffType", "").replace('"', "").strip()
                )
                if entityname != "" and not [entityname, filename] in entitylinked:
                    entitylinked.append([entityname, filename])
                    if not isModEntity(entityname + ".entity"):
                ...
                entityname = (
                    line.replace("entryVehicleType", "").replace('"', "").strip()
                )
                if entityname != "" and not [entityname, filename] in entitylinked:
                    entitylinked.append([entityname, filename])
                    if not isModEntity(entityname + ".entity"):
                ...
                entityname = (
                    line.replace("buffTypeToQuery", "").replace('"', "").strip()
                )
                if entityname != "" and not [entityname, filename] in entitylinked:
                    entitylinked.append([entityname, filename])
                    if not isModEntity(entityname + ".entity"):
            ...
            if (
                showcost
                and researchsubject
                and "Replicator_Research_Ability_Embargo.entity" not in filename
            ):
                if "pos [" in line:
                ...
                if isinstance(researchpos, int):
                    if line.startswith("Tier "):
                        ...
                        if researchpos != tier:
                            print("\tExpected Tier: " + str(
                                researchpos
                            ) + " - Tier: " + str(tier) + " incorrect for " + filename)
                    if line.startswith("BaseUpgradeTime"):
                        time = int(float(line.replace("BaseUpgradeTime", "").strip()))
                        totaltime = time
                        if researchtime[researchpos] != time:
                            print("\tExpected Time: " + str(
                                researchtime[researchpos]
                            ) + " - Time: " + str(time) + " incorrect for " + filename)
                    if line.startswith("PerLevelUpgradeTime "):
                        time = int(
                            float(line.replace("PerLevelUpgradeTime", "").strip())
                        )
                        if researchxtime[researchpos] != time:
                            print("\tExpected PerLevelUpgradeTime: " + str(
                                researchxtime[researchpos]
                            ) + " - PerLevelUpgradeTime: " + str(
                                time
                            ) + " incorrect for " + filename)
                    if line.startswith("MaxNumResearchLevels"):
                        maxlevels = int(
                            float(line.replace("MaxNumResearchLevels", "").strip())
                        )
                    ...
                    if not costcomplete and "credits " in line:
                        ...
                        if researchcredit[researchpos] != cost:
                            print("\tExpected Credits: " + str(
                                researchcredit[researchpos]
                            ) + " - Credits: " + str(
                                cost
                            ) + " incorrect for " + filename)
                        ...
                        if researchxcredit[researchpos] != cost:
                            print("\tExpected PerLevelCredits: " + str(
                                researchxcredit[researchpos]
                            ) + " - PerLevelCredits: " + str(
                                cost
                            ) + " incorrect for " + filename)
                    if not costcomplete and "metal " in line:
                        ...
                        if researchmetal[researchpos] != cost:
                            print("\tExpected Metal: " + str(
                                researchmetal[researchpos]
                            ) + " - Metal: " + str(cost) + " incorrect for " + filename)
                    elif "metal " in line:
                        cost = int(float(line.replace("metal", "").strip()))
                        if researchxmetal[researchpos] != cost:
                            print("\tExpected PerLevelMetal: " + str(
                                researchxmetal[researchpos]
                            ) + " - PerLevelMetal: " + str(
                                cost
                            ) + " incorrect for " + filename)
                    if not costcomplete and "crystal " in line:
                        ...
                        if researchcrystal[researchpos] != cost:
                            print("\tExpected Crystal: " + str(
                                researchcrystal[researchpos]
                            ) + " - Crystal: " + str(
                                cost
                            ) + " incorrect for " + filename)
    ...
    if len(shieldmesh) > 0 and len(unitmesh) > 0:
        meshgroup.append(
            {
                "unit": unitmesh,
                "entity": entityfilename.replace(".entity", ""),
                "shield": shieldmesh,
                "size": 0,
            }
        )
    ...
    if not excludebase:
        ...
        for filename in glob.glob(os.path.join(path, "*")):
            basegameparticles.append(
                os.path.basename(filename).lower().replace(".particle", "")
            )
...
if os.path.exists(os.path.join(rootpath, "entity.manifest")):
    entity_manifest = os.path.join(rootpath, "entity.manifest")
elif os.path.exists(os.path.join(basegameplaintext, "entity.manifest")):
    entity_manifest = os.path.join(basegameplaintext, "entity.manifest")
if entity_manifest:
    for line in open(entity_manifest):
...
brushmanifest = []
if os.path.exists(os.path.join(rootpath, "brush.manifest")):
...
for filename in brushmanifest:
    ...
    for line in open(filepath):
        ...
        if "fileName " in line:
            ...
            if brushname != "" and not [brushname, filename] in brushlist:
                brushlist.append([brushname, filename])
        elif line.strip().startswith("leftPlayer ") or line.strip().startswith(
            "rightPlayer "
        ):
            entityname = line.strip().split()[1].replace('"', "")
            if entityname != "" and not [entityname, filename] in entitylinked:
...
for filename in glob.glob(os.path.join(path, "*")):
    ...
    for line in open(filename):
        ...
        if "browsePictureName " in line:
            brushfilename = (
                line.replace("browsePictureName", "").replace('"', "").strip()
            )
            if brushfilename != "" and not [brushfilename, filename] in texturelist:
                texturelist.append([brushfilename, filename])
...
for filename in glob.glob(os.path.join(path, "*.randomeventdefs")):
    ...
    for line in open(filename):
        if "spawnEntity " in line:
            ...
            entityname = (
                line.replace("excludedSpawnLocation", "").replace('"', "").strip()
            )
            if entityname != "" and not [entityname, filename] in entitylinked:
                entitylinked.append([entityname, filename])
                if not isModEntity(entityname + ".entity"):
...
for filename in glob.glob(os.path.join(path, "*.constants")):
    ...
    for line in open(filename):
        if "LevelUpEffectName " in line:
            particlename = (
                line.replace("LevelUpEffectName", "").replace('"', "").strip()
            )
            if particlename != "" and not [particlename, filename] in particlelist:
                particlelist.append([particlename, filename])
        elif "EffectName " in line:
...
for filename in glob.glob(os.path.join(path, "*.explosiondata")):
    for line in open(filename):
        if "particleSystemName " in line:
            particlename = (
                line.replace("particleSystemName", "").replace('"', "").strip()
            )
            if particlename != "" and not [particlename, filename] in particlelist:
                particlelist.append([particlename, filename])
        elif "sound " in line:
...
for filename in glob.glob(os.path.join(path, "*.galaxyScenarioDef")):
    ...
    if starType != 0 and starTypeCount != starType:
        print("\t\tGalaxyScenarioDef: (starTypeCount: " + str(
            starTypeCount
        ) + ", starType: " + str(starType) + ")")
    if planetType != 0 and planetTypeCount != planetType:
        print("\t\tGalaxyScenarioDef: (planetTypeCount: " + str(
            planetTypeCount
        ) + ", planetType: " + str(planetType) + ")")
    if orbitBodyType != 0 and orbitBodyTypeCount != orbitBodyType:
        print("\t\tGalaxyScenarioDef: (orbitBodyTypeCount: " + str(
            orbitBodyTypeCount
        ) + ", orbitBodyType: " + str(orbitBodyType) + ")")
    if planetItemType != 0 and planetItemTypeCount != planetItemType:
        print("\t\tGalaxyScenarioDef: (planetItemTypeCount: " + str(
            planetItemTypeCount
        ) + ", planetItemType: " + str(planetItemType) + ")")
    if playerType != 0 and playerTypeCount != playerType:
        print("\t\tGalaxyScenarioDef: (playerTypeCount: " + str(
            playerTypeCount
    ...
    if planetItemsTemplate != 0 and planetItemsTemplateCount != planetItemsTemplate:
        print("\t\tGalaxyScenarioDef: (planetItemsTemplateCount: " + str(
            planetItemsTemplateCount
        ) + ", planetItemsTemplate: " + str(planetItemsTemplate) + ")")
    if pictureGroup != 0 and validPictureGroups != pictureGroup:
        print("\t\tGalaxyScenarioDef: (validPictureGroups: " + str(
            validPictureGroups
        ) + ", pictureGroup: " + str(pictureGroup) + ")")
    if themeGroup != 0 and validThemeGroups != themeGroup:
        print("\t\tGalaxyScenarioDef: (validThemeGroups: " + str(
            validThemeGroups
        ) + ", themeGroup: " + str(themeGroup) + ")")
...
for filename in glob.glob(os.path.join(path, "*")):
    ...
    for line in open(filename):
        ...
        if "textureName " in line:
            ...
            brushfilename = (
                line.replace("textureAnimationName", "").replace('"', "").strip()
            )
            if brushfilename != "" and not [brushfilename, filename] in texturelist:
                texturelist.append([brushfilename, filename])
        elif "MeshName " in line:
...
def readMesh(filename):
    ...
    for line in open(filename):
        ...
        if "NumPoints" in line:
            ...
            particlename = (
                line.strip().split()[1].replace('"', "").replace("Flair-", "").strip()
            )
            if particlename != "" and not [particlename, filename] in particlelist:
                particlelist.append([particlename, filename])
        elif "DiffuseTextureFileName " in line:
            brushfilename = (
                line.replace("DiffuseTextureFileName", "").replace('"', "").strip()
            )
            brushfilename = (
                brushfilename.replace("-cl.", ".")
                .replace("-da.", ".")
                .replace("-nm.", ".")
                .replace("-bm.", ".")
                .strip()
            ...
            brushfilename = (
                line.replace("SelfIlluminationTextureFileName", "")
                .replace('"', "")
                .strip()
            )
            brushfilename = (
            ...
            brushfilename = (
                line.replace("NormalTextureFileName", "").replace('"', "").strip()
            )
            brushfilename = (
                brushfilename.replace("-cl.", ".")
                .replace("-da.", ".")
            ...
            brushfilename = (
                line.replace("DisplacementTextureFileName", "").replace('"', "").strip()
            )
            brushfilename = (
                brushfilename.replace("-cl.", ".")
                .replace("-da.", ".")
            ...
            brushfilename = (
                line.replace("TeamColorTextureFileName", "").replace('"', "").strip()
            )
            brushfilename = (
                brushfilename.replace("-cl.", ".")
                .replace("-da.", ".")
    ...
    if not "Cosmetic" in group.get("entity"):
        ...
        if not os.path.isfile(shieldfile):
            shieldfile = os.path.join(
                basegameplaintext, "Mesh", group.get("shield") + ".mesh"
            )
...
for group in meshgroupsize:
    print((
        str(group.get("size")) + "\t" + group.get("entity") + "\t" + group.get("shield")
    ))
...
if not verbose:
    ...
    for item in particlefiles:
        ...
        for listitem in particlelist:
            if listitem[0].lower().replace(".particle", "") == item.lower().replace(
                ".particle", ""
            ):
                test = True
                break
    ...
    for item in meshfiles:
        ...
        for listitem in meshlist:
            if listitem[0].lower().replace(".mesh", "") == item.lower().replace(
                ".mesh", ""
            ):
                test = True
                break
    ...
    for tex in textures2:
        # Base game defaults
        if (
            tex.startswith("Cursor")
            or tex.startswith("LoadingSplash")
            or tex.startswith("Logo_Diplomacy")
        ):
            continue
        # Extra but keep
        if (
            tex == "BlackHole.dds"
            or tex == "pulsar_texture.dds"
            or tex == "sgi_logo.tga"
        ):
            continue
        ...
        for file in texturelist:
            if file[0].lower().replace(".tga", "").replace(
                ".dds", ""
            ) == tex.lower().replace(".tga", "").replace(".dds", ""):
                test = True
                break
        if not test:
...
for tex in texturelist:
    ...
    if len(extension[1]) > 0:
        ...
        if (
            not test
            and not tex[0].lower().replace(".tga", "").replace(".dds", "")
            in basegametextures
        ):
            for basetex in basegametextures:
                if basetex.replace("-cl", "").replace("-da", "").replace(
                    ...
                    break
            if not test:
                print('\t"' + str(tex[0]) + '"' + ' listed in "' + str(tex[1]).replace(
                    rootpath, ""
                ) + '" does not appear to exist in ' + dirstr + " folder.")
    else:
        for file in textures2:
...
for sound in soundlist:
    ...
    if not test:
        print('\t"' + str(sound[0]) + '"' + ' listed in "' + str(sound[1]).replace(
            rootpath, ""
        ) + '" does not have a sounddata entry.')
...
for sound in soundfilelinks:
    ...
    if not test and not sound[0].lower() in basegamesounds:
        print('\t"' + str(sound[0]) + '"' + ' listed in "' + str(sound[1]).replace(
            rootpath, ""
        ) + '" does not appear to exist in Sound folder.')
...
for item in particlelist:
    ...
    if not test and not item[0].lower() in basegameparticles:
        print('\t"' + str(item[0]) + '"' + ' listed in "' + str(item[1]).replace(
            rootpath, ""
        ) + '" does not appear to exist in Particle folder.')
...
for item in brushlist:
    ...
    if not test:
        print('\t"' + str(item[0]) + '"' + ' listed in "' + str(item[1]).replace(
            rootpath, ""
        ) + '" does not appear to exist in a Brush file.')
...
if graph:
    ...
    for item in particlelist:
        file.write(
            "Particle, "
            + os.path.basename(os.path.splitext(str(item[0]))[0])
            + ", Reference, Entity, "
            + os.path.basename(os.path.splitext(str(item[1]))[0])
            + ", 1\n"
        )
    ...
    for item in particlefiles:
        file.write(
            "Particle, " + os.path.basename(os.path.splitext(str(item))[0]) + "\n"
        )
    for item in textures2:
        if str(item).endswith("texanim"):
            file.write(
                "TextureAnimation, "
                + os.path.basename(os.path.splitext(str(item))[0])
                + "\n"
            )
        else:
            file.write(
                "Texture, " + os.path.basename(os.path.splitext(str(item))[0]) + "\n"
            )
    for item in meshfiles:
